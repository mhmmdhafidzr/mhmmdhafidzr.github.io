...
We create another variable to show how long a person use Cyclistic bike. We also had to manipulate the negative time values. Negative time values happens when a person end time bike use is less than the start time bike use. **For example**: A person start using bike at 23.00 (11 PM) and end at 02.00 next day (2 AM), this kind of scenario will have a negative time values if we use the regular calculation (end time-start time), so we have to manipulate the data. We will create a new hour, minutes, and second (hms) variable called **time** with the value of "23:59:59" and create a new calculation: **time+(end time-start time)**.

```{r}
#Create ride length and manipulate the negative time values
time=as_hms("23:59:59")
cyclist <- cyclist %>% mutate(ride_length_mins = ifelse(ended_at>started_at, 
                                                        difftime(ended_at, started_at, units='mins'), 
                                                        (time+(difftime(ended_at, started_at, units='mins')))))
```

Then we check if there's still a negative value in ride_length_mins column.
...
We found out that there's one negative value in ride_length_mins column so next we have to check the data.

```{r}
cyclist %>% filter(ride_length_mins<0) %>% select(started_at, ended_at)
```

We found out that the end date is September 21st, 2022 and the start date is September 28th, 2022, which doesn't make sense. So we have to remove this data.

```{r}
cyclist <- cyclist %>% filter(ride_length_mins>0)
```
